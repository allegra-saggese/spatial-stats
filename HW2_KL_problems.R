...
###############################################################################
# PROBLEM 4: True K-L approximation
...
# Above used s=0; but rho(tau) shouldn't depend on s. To avoid issues, evaluate
# C(s, s') for general s, s' and check stationarity.
KL_cov_matrix <- function(s_pts, KL, L) {
  w1 <- KL$w1; w2 <- KL$w2
  lam1 <- KL$lam1; lam2 <- KL$lam2
  norm1 <- sqrt(L + sin(2 * w1 * L) / (2 * w1))
  norm2 <- sqrt(L - sin(2 * w2 * L) / (2 * w2))
...
true_corr <- function(tau, phi = 1) exp(-phi * abs(tau))

# Compare for several truncation orders
orders <- c(5, 10, 25, 50)
...
###############################################################################
# PROBLEM 5: "Page 13" approximation  lambda_j ~ f(j pi/(2L))
# Spectral density of exponential correlation in 1D:
#   f(k) = (1/pi) * phi / (k^2 + phi^2)
# Eigenfunctions: psi_j(t) ~ c * exp(i j pi t / (2L))
# For real reconstruction, use cosine/sine pairs at frequencies w_j = j pi / (2L)
###############################################################################

f_exp_spectrum <- function(k, phi = 1) (1/pi) * phi / (k^2 + phi^2)

KL_page13_corr <- function(tau, N, L, phi = 1) {
  # Frequencies w_j = j pi / (2L), j = 1, ..., N
  j <- 1:N
  w <- j * pi / (2 * L)
  lam <- f_exp_spectrum(w, phi)

  # Eigenfunctions are complex exponentials e^{i w t} / sqrt(2L)
  # rho(tau) = (1/sigma^2) sum_j lam_j |psi_j(0)|^2 e^{i w_j tau}  is not quite right
  # The clean version: C(s, s') ~ sum lam_j (1/(2L)) exp(i w_j (s' - s))
  # Take real part for real correlation.
...
for (N in orders) {
  ...
  plot(tau_grid, true_corr(tau_grid), type = "l", lwd = 2, col = "black",
       ...
       main = paste("Page-13 spectral approx: N =", N))
...
###############################################################################
# PROBLEM 6: Simulate 100 realizations of GP with exponential correlation,
...
lam_spec <- sort(f_exp_spectrum(w_spec, phi) * 2 * pi, decreasing = TRUE)
# (factor 2*pi rescales density-based estimate to match operator eigenvalue scale;
#  exact constant depends on normalization conventions)
...
KL_funs <- build_KL_eigfuns(s_disc, KL_full, L)

par(mfrow = c(2, 2))
for (j in 1:4) {
  # Sign-align: empirical eigenvectors arbitrary in sign
  emp <- psi_emp[, j]
  ana <- KL_funs$funs[, j]
